...
EXAMPLES
  ...
  save: `NAME
    save — generate a portable D3CYPH3R progress code

SYNOPSIS
    save

DESCRIPTION
    Encodes the current session's progress (visited levels, earned
    achievements, bonus finds, milestones, per-level times, hint
    counters, theme, onboarding flag, lobby-tree expand state) into
    a self-contained string the player can paste into D3CYPH3R
    running on any other browser/device to resume from the same
    point. Stateless — no server, no account, no telemetry. The
    code lives wherever the player puts it (notes app, email to
    themselves, paper).

    What's NOT included: the persistence opt-in flag (per-device
    privacy choice), and any sensitive content. Codes don't contain
    level passwords or bonus content. Knowing the player visited
    "level3@linux" doesn't reveal how to solve "level2@linux".

    Pair with 'restore' to apply a code on another device. See
...
DESCRIPTION
    Decodes a progress code (generated by 'save' on any device) and
    REPLACES the current session's progress with the code's
    contents. Before overwriting, the command prints a diff
    (current vs. restored) and prompts for [y/N] confirmation.
    Typing 'y' or 'yes' applies the payload and reloads the page to
    boot into the restored state; anything else cancels.

    --preview decodes + summarizes the code without touching state.
    Use it to sanity-check a code (yours, a friend's, an old note)
    before committing to overwrite.

    Codes are typically long; paste-don't-type. Hyphens and line
    breaks inside the code are decorative — the decoder ignores
