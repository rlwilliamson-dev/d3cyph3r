...
**Safeguard 4.5 — Implement and Manage a Firewall on Servers.** Implementation tier IG1 (the entry-level tier — applies to organizations with limited security maturity). The safeguard requires a host-based firewall (Windows Defender Firewall, iptables/nftables on Linux, etc.) configured to deny all by default and allow only essential services. The remediation Atlas should have had in place is a host-level rule on the staging postgres server denying 5432 from any source IP not in the Atlas-VPN range.
