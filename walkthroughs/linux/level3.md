...
The prompt reads `daniel@halton-build-runner`. Read it again, because it is not the prompt you have had before. You are not `audit` any more. You are logged in **as Daniel**, the consultant who rolled off this engagement on 2026-01-31 and whose account nobody disabled on the bastion (level2's first bonus find) and, as you are about to confirm, nobody disabled here either. Two boxes, same oversight. That is no longer a slip; it is how Halton offboards.
...
Take the line apart, because every token in it is a finding:

- **`(root)`** means the command runs as root.
- **`NOPASSWD:`** means no prompt. Whoever lands on this account needs nothing further. This is what converts "I have Daniel's shell" into "I have a root-level primitive," at zero cost.
- **`/usr/bin/cat /var/backups/halton-prod/*`** is `cat`, constrained to a path. It reads as root, and it does not obviously hand anyone a shell. It looks narrow, which is precisely why it survived. Whoever reviewed sudoers, if anyone did, saw "cat a backup directory" and moved on to the next line.

Now look at the `*`, because that is the entire problem. A wildcard over a directory means "anything that ever lands here, for as long as this line exists." Daniel does not control what gets written into `/var/backups/halton-prod/`. The weekly cron does. So the grant's real scope is not "the files that were there when somebody approved this," it is "whatever the backup job decides to put there, forever." Least privilege under NIST AC-6 means the narrowest command on the narrowest path. A wildcard over a directory whose future contents you cannot see is the opposite of that, and it was approved because on the day it was written it happened to be harmless.
...
[^t1548-003]: [MITRE ATT&CK — T1548.003: Abuse Elevation Control Mechanism: Sudo and Sudo Caching](https://attack.mitre.org/techniques/T1548/003/).
[^gtfobins-sudo-suid-abuse-reference]: [GTFOBins — sudo/SUID abuse reference (search any binary)](https://gtfobins.org/).
[^gtfobins-cat-root-owned-file]: [GTFOBins — cat (root-owned file read via sudo)](https://gtfobins.org/gtfobins/cat/).
[^cve-2021-3156]: [CVE-2021-3156 "Baron Samedit" — Qualys advisory (26 Jan 2021)](https://blog.qualys.com/vulnerabilities-threat-research/2021/01/26/cve-2021-3156-heap-based-buffer-overflow-in-sudo-baron-samedit).
[^cve-2021-3156-nvd-detail]: [CVE-2021-3156 — NVD detail](https://nvd.nist.gov/vuln/detail/CVE-2021-3156).
[^cve-2019-14287]: [CVE-2019-14287 — sudo runas `-u#-1` bypass (NVD)](https://nvd.nist.gov/vuln/detail/CVE-2019-14287).
