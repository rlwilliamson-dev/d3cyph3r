...
CISA's published advisory on the Uber incident specifically named **MITRE ATT&CK T1552.001 (Unsecured Credentials: Credentials In Files)** as the lateral-movement technique. The advisory's recommended mitigations are exactly the controls level1@linux's remediation list specifies: secrets-management deployment with no cached credentials, file-integrity monitoring on operational scripts, employee training on the difference between *the credential's canonical protected location* and *cached copies thereof*.
...
- [CISA Advisory — Uber breach lateral-movement techniques](https://www.cisa.gov/news-events/cybersecurity-advisories)
