...
**CWE-732** is the exact weakness category here.[^cwe-732] MITRE's wording: *"The product specifies permissions for a security-critical resource in a way that allows that resource to be read or modified by unintended actors."* A production database password, in a world-readable file, at mode 644. You could put Daniel's `.bak` in the catalog as the illustration.

The entry dates to 2006 and appeared on the Top 25 Most Dangerous Software Weaknesses in 2020 and 2021. It has since fallen off the annual list while remaining actively maintained and heavily mapped in the real world, which by now should sound familiar. Its mapping status is **ALLOWED-WITH-REVIEW**, the second-strongest tier, and MITRE's caution is that people reach for it when they mean an authorization problem rather than a permission-assignment one. That caveat does not apply to us: a `cp` produced the wrong mode, which is permission assignment in its purest form. CWE-200 and CWE-668 are both marked Discouraged for direct mapping, so when the formal report goes to Halton, CWE-732 is the citation that holds up.[^cwe-668][^cwe-200]

MITRE also suggests the mitigation, and it is more useful than it first appears: *"When using a critical resource such as a configuration file, check to see if the resource has insecure permissions, e.g., world-writable... If insecure permissions are detected, warn the user and/or exit the program."* In other words, make the software refuse to start. §7 turns that into something you can actually deploy.

Two neighbours in the family are worth recognising. CWE-276, Incorrect Default Permissions, is when the defaults at creation time are wrong. CWE-281, Improper Preservation of Permissions, is when a tool copies a file and drops the source's restrictions on the floor.[^cwe-276][^cwe-281] CWE-732 is the umbrella over both, and if you want to be precise about Halton, CWE-281 is the one: `cp` did not carry mode 600 across, and the destination took the session umask's 644 instead.
...
**AC-3, Access Enforcement.** The system must enforce approved authorizations for logical access. Note that the mode-600 `staging-worker.env` file is AC-3 succeeding: the enforcement layer looked at your read attempt and refused it, correctly. The failure is next door, on the shadow copy, where the same enforcement layer cheerfully permits the read because somebody told it 644. AC-3 asks for the enforcement decision to be *right for the resource*, not merely consistent. Permitting any user on the box to read production credential material is a wrong decision, consistently applied.

**AC-6, Least Privilege.** Subjects get only the privileges their authorized tasks require, and two failures stack up here. `app_admin` runs the staging-worker daemon, which talks to the staging database. It has no business being able to read a *production* credential at all, and the shadow copy handed it one. Separately, `app_admin` holds unrestricted sudo, which is its own violation, since the account's real needs amount to restarting its own service and reading its own logs.

Of AC-6's 13 enhancements, the one that stings is **AC-6(2), Non-Privileged Access for Nonsecurity Functions**. Privileged accounts are for security functions; everything else uses a non-privileged one. Daniel used sudo, a privileged mechanism, to make debugging slightly more convenient, which is a non-security task by any reading. Textbook AC-6(2), and the kind of thing that feels harmless in the moment on a Tuesday.

**SC-28, Protection of Information at Rest.** Information at rest needs confidentiality and integrity protection, via cryptographic means (SC-28(1)) or physical controls. Unencrypted credentials in a flat file fail this regardless of permissions, which means the legitimate override file is *also* an SC-28 finding if the host filesystem is not encrypted at rest. The shadow copy just serves the same content to a wider audience.

Resist the urge to fix SC-28 by adding more files with tighter modes. The credential should not be on the filesystem at all. Secrets managers fetch at runtime, keep the value in process memory, and never write it down. That is the compliant pattern. An override file at mode 600 is a compromise that passes SC-28 only if you squint and the disk is encrypted.
...
Diagnosis is the easy half. This is the part where somebody has to phone a bank. Every defender at a financial institution, and every engineer running production-bastion infrastructure anywhere, eventually works a finding shaped like this one.
...
Fed into Halton's SIEM, this fires the next time somebody `sudo cp`s a sensitive system file into a home directory. Think about what that buys. Daniel's copy existed for five months before an outside auditor tripped over it. This rule turns five months into five minutes, and it does not require anyone to be more careful than they were.
