...
Published May 14, 2024 (Final). The current standard Polaris is audited against. The §03.03 (Audit and Accountability) family inherits directly from 800-53 AU controls, tailored for non-federal systems. Note that Rev. 3 adopted a zero-padded, period-separated numbering scheme (`03.03.01`) — distinct from Rev. 2's `3.3.1` style:
...
- **EvtxECmd** (ericzimmerman.github.io) — Eric Zimmerman's EZ Tools command-line .evtx parser. The go-to for offline triage.
- **Hayabusa** (github.com/Yamato-Security/hayabusa) — Yamato Security's threat-hunting tool that ships with thousands of pre-built detection rules in Sigma format, applied to .evtx files in bulk.
- **Chainsaw** (github.com/WithSecureLabs/chainsaw) — WithSecure Labs' tool that searches .evtx files using YAML detection rules. Often used alongside Hayabusa for cross-validation.
...
- **NIST SP 800-53 Rev. 5 — Security and Privacy Controls for Information Systems and Organizations**: <https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final>. Published September 2020; Revision 5 Update 1 published December 2023. AU family is in chapter 3.3.
...
- **CWE-532: Insertion of Sensitive Information into Log File**: <https://cwe.mitre.org/data/definitions/532.html>. The defender-side mapping of the 4625 typed-password finding.
- **CWE-117: Improper Output Neutralization for Logs**: <https://cwe.mitre.org/data/definitions/117.html>. Adjacent; covers log-injection.
- **CWE-200: Exposure of Sensitive Information to an Unauthorized Actor**: <https://cwe.mitre.org/data/definitions/200.html>. Parent of CWE-532. Note: CWE-200's mapping status is currently **Discouraged** — cite CWE-532 for direct mappings.
...
- **Twitter incident report (July 2020)**: <https://blog.x.com/en_us/topics/company/2020/an-update-on-our-security-incident>. Reconstruction from internal audit logs. (Twitter blog migrated to X; original `blog.twitter.com/...` URL redirects here.)

### Practitioner blogs / training material
